...
export function TwistyControls({ twistyPlayer, className }: { twistyPlayer: TwistyPlayer; className?: string }) {
  ...
  return (
    <div className={cn('flex gap-9', className)}>
      ...
      <button
        className='outline-ring transition-base flex h-16 w-16 items-center justify-center rounded-full bg-primary-60 text-black-100 hover:bg-primary-80 active:bg-primary-60'
        onClick={playPause}
      >
...
function ControlButton({ className, children, ...props }: HTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...props}
      className={cn(
        'transition-base outline-ring flex h-14 w-14 items-center justify-center text-[1.5rem] hover:text-primary-60 active:text-primary-80',
        className,
      )}
    >
      {children}
